...
let
  t = pkgs.writeShellScriptBin "t" ''
    ...
    if ! tmux has-session -t=main 2> /dev/null; then
      ...
      tmux new-window -t main -n home-manager -c ~/home-manager/
    fi
    tmux attach -t main:1
  '';
  tcs = pkgs.writeShellScriptBin "tcs" ''
    #!/usr/bin/env bash
    ...
    if [[ -z $TMUX ]] && [[ -z $tmux_running ]]; then
        ...
        tmux new-window -t ''${selected_name} -n ia -c ''${selected}
        tmux send-keys -t ''${selected_name}:ia "gemini" C-m
